...
describe("ig-actor-validator does not double-prefix the ad account id (regression: 1772103)", () => {
  ...
  it("constructs a URL with exactly one /act_ segment (never /act_act_)", async () => {
    let requestedUrl = "";
    mock.method(global, "fetch", async (input: string | URL | Request) => {
      requestedUrl = typeof input === "string" ? input : input.toString();
    ...
    // Matches `/act_` only when NOT followed by another `act_` — i.e. a single,
    // correctly-formed account segment. Double-prefix (`/act_act_`) fails this.
    assert.match(
      requestedUrl,
      /\/act_(?!act_)/,
      `URL must contain exactly one /act_ segment; got: ${requestedUrl}`,
    );
  });
