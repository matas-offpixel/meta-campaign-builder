...
describe("lastAiTagAt", () => {
  it("returns the latest created_at for AI rows of the given model", () => {
    const rows = [
      row("ai", SONNET, "2026-05-20T10:00:00Z"),
      row("ai", SONNET, "2026-05-22T08:00:00Z"),
      row("ai", SONNET, "2026-05-21T09:00:00Z"),
    ];
    assert.equal(
      lastAiTagAt(rows, SONNET)?.toISOString(),
  ...
  it("ignores other sources and other model versions", () => {
    const rows = [
      ...
      row("ai", OTHER_MODEL, "2026-05-22T11:00:00Z"),
      row("ai", SONNET, "2026-05-19T07:00:00Z"),
    ];
    assert.equal(
      lastAiTagAt(rows, SONNET)?.toISOString(),
  ...
  it("returns null when the model has never tagged this event", () => {
    const rows = [row("ai", OTHER_MODEL, "2026-05-22T11:00:00Z")];
    assert.equal(lastAiTagAt(rows, SONNET), null);
  });

  it("skips unparseable timestamps", () => {
    const rows = [
      row("ai", SONNET, "not-a-date"),
      row("ai", SONNET, "2026-05-18T06:00:00Z"),
    ];
    assert.equal(
      lastAiTagAt(rows, SONNET)?.toISOString(),
