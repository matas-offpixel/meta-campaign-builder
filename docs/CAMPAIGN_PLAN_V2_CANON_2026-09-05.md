...
**Amended 2026-09-06 after the live walks** (`/plan/299dd4e5` D.O.D, `/plan/abf386e4` Jamie Jones; PRs #903–#907): the reading unit follows the phase, never the stored `target_unit`; campaign-scope (CBO) decisions name the campaign verbatim; the header and page title are `events.name`; the destination URL lives in the ⓘ as `tickets at <domain>`; blockers have one source (`collectPlanPreflightBlockers`) shown per adapter on rows and as a total on the button and fold, the fold reading `before you can launch` when they span adapters; a draft with no moment ahead is `done`; the list pace bar is solid for live plans whatever the stored end; placeholders for unset moments are one line beneath the rail, never on it; the usual outline is two ticks and a hairline; the empty purchase reading is the `£—` chip; unconnected channels read `not connected`; asset slots carry the filename; the face renders the skeleton until reads settle; ⓘ cards are one per zone. G35 added.

**Amended 2026-09-07 after frames round 3:** "ticket" is the display word for the `tickets_sold` line of the `purchase` unit (§2.2 D). A client whose result is tickets (Junction 2) divides all-channel spend by `tickets_sold` and reads **`per ticket`**; `Meta says N purchases` is the second, platform-said line beneath that chip — never a second cost-per-purchase chip. Stored `target_unit` stays `purchase` (migration 165). `VIZ_UNIT_WORD` gains `ticket`; 165's vocabulary does not.
