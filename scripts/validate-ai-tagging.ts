...
//
// Ground truth defaults to manually-applied tags (the original behaviour). For
// the Haiku-vs-Sonnet gate, set:
//   VALIDATE_GROUND_TRUTH=ai
//   VALIDATE_GROUND_TRUTH_MODEL=claude-sonnet-4-6   (the incumbent tags)
//   VALIDATE_PREDICT_MODEL=claude-haiku-4-5         (defaults to current model)
//   VALIDATE_LIMIT=200                              (optional sample cap)
// Then `precision` = % of Haiku tags that match Sonnet, `recall` = % of Sonnet
// tags Haiku reproduced, and `agreement` (F1) summarises the two per dimension.
...
async function main() {
  ...
  // Exact per-(creative × dimension) set agreement — the headline "do Haiku and
  // Sonnet land on the same answer" number, complementing the tag-level F1.
  let cellsCompared = 0;
  ...
  const output = {
    ...
    // For an AI ground truth this is the Haiku-vs-Sonnet agreement: precision =
    // share of predicted tags that the ground truth also has, recall = share of
    // ground-truth tags reproduced, agreement (= f1) the harmonic mean.
