...
--
-- Backfill from the linked drafts. Meta: campaign_drafts.settings.adAccountId
-- (then settings.metaAdAccountId). TikTok: tiktok_campaign_drafts.state
-- advertiserId / accountSetup.advertiserId. Google: google_search_plans
...
comment on column campaign_plan_meta_launch.platform_ad_account_id is
  'Ad account the Meta campaign actually launched in (draft settings.adAccountId). Identity reads this after launch, never the resolver.';
...
comment on column campaign_plan_google_launch.platform_ad_account_id is
  'Google Ads account the search plan actually launched in. Identity reads this after launch when a draft exists.';

update campaign_plan_meta_launch l
set platform_ad_account_id = coalesce(
  nullif(trim(d.settings->>'adAccountId'), ''),
  nullif(trim(d.settings->>'metaAdAccountId'), '')
)
...
set platform_ad_account_id = nullif(trim(p.google_ads_account_id::text), '')
from google_search_plans p
where l.draft_id = p.id
  and l.platform_ad_account_id is null
  and nullif(trim(p.google_ads_account_id::text), '') is not null;
