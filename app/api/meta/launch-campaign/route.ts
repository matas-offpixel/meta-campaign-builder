...
import { bindLaunchAdSetRecorder } from "@/lib/launched-ad-sets/launch-recorder";
...
// ─── Recreate-from-scratch fallback: "Meta's create validator disagrees ────
// with its own availability read endpoint" (task #124)
//
// Reproducer: IPC Newcastle Signup v3's "Similar Pages" ad set (2026-08-07
// 21:45 UTC, trace AwXXdOKyQMbDh8sbLfrquGg). The preflight check (task #123
// FIX 2) asked Meta's `/customaudiences?fields=delivery_status,operation_status`
// endpoint about 40 custom audiences, dropped the 11 it called unavailable,
// and retried with 29 "clean" survivors — Meta's ad-set-create endpoint
// rejected the retry anyway (subcode 1359207, no offending ids named). A
// SECOND availability check on those exact 29 ids (inside the CA-salvage
// loop below) reported every one of them fine. `recoverFromDeletedCa`
// correctly refuses to invent ids to drop when Meta names none and its own
// read endpoint disagrees — but that leaves the ad set permanently stuck:
// there is no read endpoint that exposes whichever stricter create-time rule
// Meta is actually applying to these specific ids. The only remaining lever
// is to stop trusting ANY existing audience id for this page group and
// create a brand-new set from scratch — Meta accepts newly-created custom
// audiences it refuses to accept as pre-existing ones (the same populating-
// window mechanics `waitForAudienceReady` already works around for audiences
// created earlier in THIS run, just applied to older ones instead).
//
// Mirrors Phase 1.5's per-(page, engagementType) creation loop but WITHOUT
// its "reuse existing audience" branch — every call below is a forced fresh
// create via createEngagementAudienceWithRecovery. Best-effort: a handful of
// individual (page, type) failures (e.g. a permission gap on one page) don't
// abort the whole recreate — the caller decides whether the resulting id
// list is usable.
async function recreateEngagementAudiencesForGroup(
  ...
  opts: {
    ...
    adAccountId: string;
    userFbToken?: string;
    pageToIg: Map<string, string>;
    pageNameMap: Map<string, string>;
    engagementLabels: Record<string, string>;
  },
): Promise<{
  ids: string[];
  ...
  const ids: string[] = [];
  const created: { name: string; id: string; type: EngagementType; pageId: string; pageName: string }[] = [];
  const failed: { name: string; type: EngagementType; error: string }[] = [];
  ...
  for (const pageId of group.pageIds) {
    for (const et of group.engagementTypes) {
      ...
      const igId = isIgType ? opts.pageToIg.get(pageId) : undefined;
      const pageName = opts.pageNameMap.get(pageId) || group.name || "Page Group";
      // Same skip rule as Phase 1.5 — no linked IG account, nothing to recreate.
      if (isIgType && !igId) continue;

      const sourceId = isIgType ? igId! : pageId;
      const sourceType = isIgType ? ("ig_business" as const) : ("page" as const);
      const audienceName = `${pageName} — ${opts.engagementLabels[et] ?? et} (recreated)`;

      try {
        const { result } = await createEngagementAudienceWithRecovery(
          opts.supabase,
          opts.userId,
          opts.adAccountId,
          {
            ...
            sourceId,
            sourceType,
            userToken: opts.userFbToken,
            pageId,
            pageName,
          },
        );
        ids.push(result.id);
        created.push({ name: audienceName, id: result.id, type: et, pageId, pageName });
      } catch (err) {
        failed.push({ name: audienceName, type: et, error: formatMetaError(err) });
      }
    }
  }
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const isAttachAllAdSets = wizardMode === "attach_all_adsets";
  ...
  const isMultiCampaignAttach =
    ...
    attachCampaignSnapshots.length > 1;
  console.log(
  ...
  if (wizardMode === "attach_campaign" || isAttachAllAdSets) {
    ...
    // GOAL 3 — cross-campaign: each ad set's campaignId must be one of the
    // selected campaigns (not just the single attachTargetId).
    const allowedCampaignIds = new Set(attachCampaignSnapshots.map((s) => s.id));
  ...
  // 0c-bis. Warn early about stale/unavailable custom audiences (task #115).
  // Batch-checks every custom-audience id referenced by an enabled ad set —
  // page-group picks + auto-created engagement audiences, custom-audience
  // groups, and saved audiences — so the operator sees "3 of 40 audiences
  // unavailable, will drop on launch" up front instead of discovering it
  // only after Phase 2 already had to salvage-retry (or fail) the ad set.
  // Non-blocking: the launch proceeds either way; Phase 2's
  // recoverFromDeletedCa ladder is what actually drops them at create time.
  {
    const preflightCaIds = new Set<string>();
    for (const adSet of enabledSets) {
      if (adSet.sourceType === "page_group") {
        const group = draft.audiences.pageGroups.find((g) => g.id === adSet.sourceId);
        for (const id of group?.customAudienceIds ?? []) preflightCaIds.add(id);
        for (const id of group?.engagementAudienceIds ?? []) preflightCaIds.add(id);
      } else if (adSet.sourceType === "custom_group") {
        const group = draft.audiences.customAudienceGroups.find((g) => g.id === adSet.sourceId);
        for (const id of group?.audienceIds ?? []) preflightCaIds.add(id);
      } else if (adSet.sourceType === "saved_audience" && /^\d{10,}$/.test(adSet.sourceId)) {
        preflightCaIds.add(adSet.sourceId);
      }
    }
    const uniqueCaIds = [...preflightCaIds].filter((id) => /^\d{10,}$/.test(id));
    if (uniqueCaIds.length > 0) {
      const statuses = await fetchCustomAudienceAvailability(uniqueCaIds, launchToken || undefined);
      const staleIds = statuses.filter((s) => !s.available).map((s) => s.id);
      if (staleIds.length > 0) {
        console.log(
          `[launch-campaign] Preflight — ${staleIds.length} of ${uniqueCaIds.length} referenced custom audience(s)` +
            ` unavailable: ${staleIds.join(", ")}`,
        );
        preflightWarnings.push({
          stage: "custom_audiences",
          message:
            `${staleIds.length} of ${uniqueCaIds.length} custom audiences referenced by your ad sets are no ` +
            `longer available on Meta's side — they will be automatically dropped at launch (see the "note" ` +
            `on the affected ad set(s) in the launch report).`,
          severity: "amber",
        });
      }
    }
  ...
  if (wizardMode === "attach_campaign" || wizardMode === "attach_adset" || isAttachAllAdSets) {
    ...
    try {
      ...
      // GOAL 1 FIX — compare each campaign's live objective against ITS OWN
      // snapshot, not against the global draft.settings.objective which can
      ...
      if (isMultiCampaignAttach && attachCampaignSnapshots.length > 1) {
        ...
        const additionalResults = await Promise.allSettled(
          additionalSnaps.map(async (snap) => {
            ...
            // objective against THIS campaign's own snapshot objective.
            if (snap.objective && c.objective && c.objective !== snap.objective) {
              throw new Error(
                `Campaign "${c.name}" objective changed since you picked it ` +
      ...
      if (wizardMode === "attach_adset") {
        ...
        // GOAL 3 — cross-campaign: orphan check against ALL selected campaigns.
        const allowedCampaignIdsP1 = new Set(attachCampaignSnapshots.map((c) => c.id));
  ...
  // ── attach_all_adsets short-circuit ─────────────────────────────────────
      ...
      for (const r of results) {
        if (r.status === "fulfilled") {
          ...
          adSetsCreated.push({
            ...
            // actually "strict" on Meta's side even though `advantagePlus`
            // is still true in the draft (unchanged — see the note).
          ...
          recordCreatedAdSet(metaCampaignId, adSet, metaAdSetId);
        } else {
  ...
  if (lookalikeSets.length > 0) {
    ...
    for (const adSet of lookalikeSets) {
      ...
      try {
        ...
        recordCreatedAdSet(metaCampaignId, adSet, adSetRes.id);
      } catch (err) {
        ...
        try {
          ...
          recordCreatedAdSet(metaCampaignId, adSet, salvaged.metaAdSetId);
        } catch (salvageErr) {
  ...
  // attach_adset / attach_all_adsets: include each synthetic key so Phase 4
  // logs the live ad set's name instead of the synthetic projection id.
  if (wizardMode === "attach_adset") {
  ...
  // attach_all_adsets: synthetic keys registered in Phase 2 only have IDs;
  // populate display names from the adSetMetaIds reverse map.
  if (isAttachAllAdSets) {
    for (const [synthKey] of adSetMetaIds) {
      if (!adSetNameById.has(synthKey)) {
        adSetNameById.set(synthKey, synthKey);
      }
    }
  }
  ...
  console.log(
    `[launch-campaign] Phase 4 — linking ads` +
      (wizardMode === "attach_adset"
        ...
        : "") +
      (isAttachAllAdSets
        ? ` (attach_all_adsets: routing all ${creativesCreated.length} creatives to ${adSetMetaIds.size} ad sets)`
        : ""),
  );
  ...
  for (const creativeEntry of creativesCreated) {
    ...
    // every fetched ad set (all-to-all: each creative × every ad set).
    const assignedAdSetIds: string[] = isAttachAllAdSets
      ? Array.from(adSetMetaIds.keys())
      : (creativeToAdSetIds[creative.id] ?? []);
    ...
    for (let ci = 1; ci < verifiedCampaigns.length; ci++) {
      ...
      for (let i = 0; i < standardSets.length; i += CI_BATCH_SIZE) {
        ...
        for (const r of batchResults) {
          if (r.status === "fulfilled") {
            ...
            recordCreatedAdSet(nextCampaign.id, adSet, metaAdSetId);
          } else {
            const reason = r.reason as { adSet: AdSetSuggestion; err: unknown };
            const message = formatMetaError(reason.err);
      ...
      for (const adSet of lookalikeSets) {
        ...
        try {
          ...
          recordCreatedAdSet(nextCampaign.id, adSet, adSetRes.id);
          ...
          try {
            ...
            recordCreatedAdSet(nextCampaign.id, adSet, salvaged.metaAdSetId);
          } catch (salvageErr) {
            const message = formatMetaError(salvageErr);
