...
import { readFourthefansEventSales } from "@/lib/ticketing/fourthefans/parse";
...
export async function POST(req: NextRequest) {
  ...
  const snapshots = await loadLatestFourthefansSnapshots(admin, {
    userId: user.id,
    eventId,
  });
  if (!snapshots.ok) {
  ...
  for (const snapshot of snapshots.rows) {
    try {
      const parsed = readFourthefansEventSales(snapshot.raw_payload);
      const capacity = dryRun
        ? {
            computedCapacity: ticketTierCapacity(parsed.ticketTiers),
            updated: false,
      ...
      results.push({
        ...
        tiers_found: parsed.ticketTiers.length,
...
async function loadLatestFourthefansSnapshots(
